...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        B,T = idx.shape
        token_emb = self.token_embedding_table(idx) #(Batch Size, Vocab Size, n_embed)
        position_emb = self.position_embedding_table(torch.arange(T, device=device)) #(Batch Size, Vocab Size)
        token_position_emb = token_emb + position_emb #(Batch Size, Vocab Size, n_embed)
        logits = self.lm_head(token_position_emb) #(Batch Size, Vocab Size, Vocab Size)
